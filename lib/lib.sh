...
# FNPanel: pipefail so a failing curl in "curl | grep | sed" pipelines is not
...
export SCRIPT_RELEASE=${SCRIPT_RELEASE:-FNPanel}
...
export GITHUB_BASE_URL=${GITHUB_BASE_URL:-"https://raw.githubusercontent.com/FuryNocturn/FNPanelControlServer"}
...
create_db_user() {
  ...
  # IF NOT EXISTS keeps the installer re-runnable (idempotent) instead of
  # aborting under 'set -e' when the user already exists.
