...
# FNPanel: load timesync (chrony) helpers so this node can sync from the panel.
if ! fn_exists configure_timesync_client; then
...
# FNPanel: NTP client (sync clock from panel)
CONFIGURE_TIMESYNC_CLIENT="${CONFIGURE_TIMESYNC_CLIENT:-false}"
TIMESYNC_PANEL_HOST="${TIMESYNC_PANEL_HOST:-}"
...
configure_mysql() {
  ...
  if [ "$MYSQL_DBHOST_HOST" != "127.0.0.1" ]; then
    ...
    # Service is mariadb on both Debian/Ubuntu and RHEL mariadb-server (not mysqld).
...
perform_install() {
  ...
  # FNPanel: sync this node's clock from the panel (chrony client).
  if [ "$CONFIGURE_TIMESYNC_CLIENT" == true ] && fn_exists configure_timesync_client; then
    configure_timesync_client "$TIMESYNC_PANEL_HOST"
  fi
